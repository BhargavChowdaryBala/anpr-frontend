...
function App() {
    ...
    const handleImageCheck = async (e) => {
        ...
        try {
            ...
            // Adapt to expected UI format
            setAnalysisResult({
                confidence: apiResult.confidence ? Math.round(apiResult.confidence * 100) : 0,
                plate_text: apiResult.license_plate || "No Plate Detected",
                bus_detected: apiResult.bus_detected,
                plate_image: null,
                raw_texts: apiResult.license_plate ? [apiResult.license_plate] : [],
                metrics: { yolo: "Remote", ocr: "Remote" }
            });
        } catch (err) {
            console.error("Analysis Error:", err);
            alert("Failed to analyze image. Ensure Backend and ML service are running.");
        } finally {
